# Wire-Up Plan — Earnings & Catalyst Dashboard
...
**Option A — Vercel Deployment Protection (recommended, zero code).**
- Enable Deployment Protection in the Vercel project settings.
- Skip everything below for this phase; move straight to W1.

**Option B — Single access code (if you want to share the URL without a
Vercel account).**
- Add `middleware.ts` that redirects unauthenticated requests to `/login`.
- One env var: `ACCESS_CODE`.
- `POST /api/auth/verify` accepts `{ code }`, on match sets an httpOnly
  session cookie signed with an internal secret.
...
FE changes (Option B only):
- `RoleProvider` fetches `/api/auth/me` on mount (returns just `{ ok: true }`
  when the cookie is valid; no role split for personal use).
- `LoginPage` posts to `/api/auth/verify`.

**Debug areas (Option B):**
- Session-cookie domain settings differ between preview URLs
  (`*.vercel.app`) and any custom prod domain. Cookie must be scoped
  correctly or the login flow will loop.

**Testing checkpoint W0.T1:**
1. Curl `/api/health` (unauthenticated) → 200 with `ok: true`.
2. Option A: hitting the URL from a browser without a Vercel session →
   Vercel-hosted login page. Same URL from your logged-in browser → app
   renders.
3. Option B: `GET /` unauthenticated → 302 to `/login`. Correct code →
   cookie present → app renders. Wrong code → inline error.
4. Restart the server → session persists.

**Rollback**: disable Deployment Protection (Option A) or revert middleware
(Option B). FE remains usable.
...
**Data-transformation risks / debug areas:**
- **GitHub Contents API 409 storms.** For personal single-user use these
  are rare, but tab-race conditions (edit the same event from two tabs)
  can still trigger them. The retry loop must re-fetch, re-merge on the
  correct field, and not clobber the other tab's unrelated changes.
...
1. Write a verdict note; ~1.5s later a commit appears in git history with
   the correct message pattern.
2. Refresh the page; the note persists.
3. Open the same event in a second tab and save a different note within
   2 seconds → later one wins, earlier tab receives a 409-toast and
   re-fetches.
4. Toggle `GH_PAT` off (rename in env) → banner appears within 60 s;
   attempting to save writes to localStorage only; no data loss on the
   subsequent toggle-back.
5. Feedback: mark an item not-relevant → new entry appears in
...
7. Session cookie missing → `POST /api/entity-registry` returns 401.
...
- **No Cloudflare Worker in this build (DC15).** `/api/tweets` implements the
  TwitterAPI.io path only, and returns `EngineStatus.ok=false, itemsFound=0`
  when `TWITTERAPI_IO_KEY` is unset. X via Nitter is dropped.
...
- **The single most fragile area.** Vendor quirks per Appendix A:
  - **EDGAR** requires a `User-Agent` header carrying a real contact email
    (`Earnings Tracker <you@example.com>`). Missing → 429 within seconds.
...
- **`mentionsHolding` false positives** (`$BN` → Bambi). B.1 exclusion
  aliases must actually be applied by the tweet path (only if TwitterAPI.io
  is enabled).
...
4. `GET /api/tweets?ticker=BN%20US` returns
   `{ items: [], engineStatus: [{engine:"twitter", ok:false, itemsFound:0}] }`
   when `TWITTERAPI_IO_KEY` is unset — treated as an expected empty bucket,
   not a failure. If the key is set, tweets return via TwitterAPI.io only.
...
- **Mobile read acceptance**. Journey J6 (mobile Overview + Detail on a
  real phone) — confirm the responsive collapse works.
- **Investigate acceptance**. Journey J2 (investigate a print, verify from
...
3. All six journeys from FE PRD §8 pass end-to-end on the live app:
   ...
   J5 add custom source, J6 mobile read.
...
6. **Session cookie domain on preview vs prod URLs (W0).**
...
|---|---|---|
| W0 | 1 day | pick Vercel Deployment Protection or `ACCESS_CODE` |
| W1 | 2 days | W0 |
| W2 | 3–4 days | W1 |
...
**Total: ~4–6 weeks** to go-live. Parallelization is possible in W7 (hosted
docs) alongside W6 (cron).

The only prereq to start: create a fine-grained `GH_PAT` with `Contents:
