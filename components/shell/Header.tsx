...
const TABS = [
  ...
  { href: "/admin", label: "Admin", editorOnly: true },
];

export function Header() {
  const path = usePathname();
  const { theme, cycle } = useTheme();
  const { isEditor, user, toggle, role } = useRole();
  ...
  return (
    ...
    >
      <div className="flex items-center gap-6">
        ...
        <nav className="flex items-center gap-[4px]" aria-label="Primary">
          {TABS.map((t) => {
            ...
            if (t.editorOnly && !isEditor) return null;
            const active =
      ...
      <div className="flex items-center gap-3">
        ...
        <button
          onClick={toggle}
          className="flex h-8 items-center gap-2 rounded-button border border-bd bg-panel px-2 text-[12px] text-tx2 hover:text-tx"
          title={`Toggle role (currently ${role})`}
        >
          <span className="flex h-[24px] w-[24px] items-center justify-center rounded-full bg-s3 text-[10.5px] font-semibold text-tx2">
            {user.initials}
          </span>
          <span className="pr-1 text-tx3">
