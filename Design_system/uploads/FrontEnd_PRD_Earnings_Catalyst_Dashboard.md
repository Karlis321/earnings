# Front-End PRD — BluOr Earnings & Catalyst Dashboard
...
In scope: all read views (overview, security detail for the three security types, event detail, source viewer, sector view) and all editor views (add/edit security, manual entry, custom sources, feedback). Out of scope: the ingestion/cron internals (project PRD §6–9), and any external/marketing surface (this is an internal tool behind auth).

Two roles shape every view:
- **Analyst / Maintainer (editor):** full read + all admin surfaces.
- **CIO / PM (read-only):** all read views; admin is hidden and blocked server-side.

---

## 2. Design Principles & Constraints

- **Desktop-first.** The analyst workflow is dense-table + detail on a wide screen. Mobile is a functional read-only view (overview + detail) for the CIO; admin/entry is desktop-only.
...
### 7.1 Login / Auth Gate
- **Purpose:** restrict the app to BluOr users; establish role.
...
- **Elements:** ticker + period selector; metric selector (from the security's headline metrics); value + unit; **source (required)**; **as-of (required)**; method (defaults bloomberg_manual); confidence (optional); note. A running list of manual values for this ticker with edit/supersede.
