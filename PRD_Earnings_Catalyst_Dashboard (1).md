# Product Requirements Document — BluOr Earnings & Catalyst Dashboard

*Working name. Internal BluOr Asset Management tool. Draft v0.3.*

*v0.2 note: the project will ship as a NEW git repository, but it reuses the proven API connections from an existing BluOr news-tracker codebase. Section 9 and Appendices A–B document those connections in port-ready detail so the new build replicates them exactly rather than rediscovering each vendor quirk. Where v0.1 assumed a data engine and storage from scratch, v0.2 corrects them against what the reference code actually does.*
...
The Earnings & Catalyst Dashboard is an internal BluOr tool that tells a portfolio analyst, for every covered name, when it reports, whether it beat or missed, how guidance moved, what management said, and how the stock reacted — with a one-click path to the exact spot in the primary source so any number can be verified without leaving the app. The target user is the BluOr portfolio analyst who currently rebuilds this picture by hand across filings, transcripts, and Bloomberg each earnings season, with the CIO and other PMs as secondary readers. The value proposition is a single, always-current, fully-sourced view that turns a multi-hour manual gathering exercise into a glance, while preserving the discipline of tracing every number back to its filing. Version 1 covers 17 deeply-tracked names; later phases extend to a shallow S&P 500 / EuroStoxx headline scan and sector-level "vibe and forward" reads. Because the book spans producers, pre-revenue developers, and index ETFs, the product models three distinct security types rather than forcing everything into an "earnings" mold.
...
| Secondary adoption | CIO/PM opens the dashboard ≥3×/week | *[ASSUMPTION]* |
...
**Primary — Toms, Portfolio Analyst**
- Role: builds investment research (evaluation reports, valuation models, tearsheets) at BluOr; owns coverage of the 17 names.
- Pain point: every earnings season he manually reassembles report dates, consensus, actuals, guidance, production, and price reaction across filings, transcripts, and Bloomberg — slow, repetitive, and easy to let a number go stale.
- Gains: a single current view per name; instant beat/miss and guidance-move read; one click to the exact transcript paragraph to verify; accumulated history he never has to rebuild.

**Secondary — CIO / Portfolio Manager**
- Role: consumes the analyst's coverage at a higher level; makes allocation calls.
- Pain point: has to wait for the analyst to compile and summarize; can't self-serve a quick "what happened this quarter and how did the stock take it."
- Gains: a read-only, plain-English view of each name's latest print, guidance move, and reaction; later, a sector "vibe + forward" summary.

**Secondary — Data Maintainer (may be the same analyst)**
- Role: keeps the small manual layer current — the fields FMP can't cover (small-cap consensus, NAV) entered from Bloomberg.
- Pain point: manual entry is where staleness creeps in and where sourcing discipline breaks.
- Gains: a structured admin entry surface that forces source + as-of on every manual value, so the ledger stays intact.
...
- User Story: As the CIO, I want a plain-English sector read so that I can grasp the theme without reading every print.
- Acceptance Criteria:
...
**M1. Access control**
- Description: gate the app to BluOr users; read-only role for CIO/PM, editor role for maintainer.
...
- Open Questions: `[NEEDS DECISION]` auth provider and whether SSO with BluOr identity is required vs a simple access list.
...
- **Required request headers (reference-proven, easy to get wrong):** SEC EDGAR requires a `User-Agent` carrying a contact (`BluOr News Tracker <email>`); Q4-hosted IR feeds (Century, Hudbay) and several vendor IR platforms `403` anything that isn't a real browser UA + `Accept-Language` — sending a current Chrome UA turns `403→200`. B3 Mziq requires POST + a known `Origin`/`Referer` despite a misleading `401` on naive GETs.
...
- **Responsive:** functional read-only view on mobile for the CIO (overview + detail); admin/entry is desktop-only.
...
This is an internal BluOr tool, so there is no customer-facing pricing. The relevant model is **operating cost**, kept at/near zero for v1:
...
- Mobile-first read view for the CIO.
...
- **Required** `User-Agent` with contact, e.g. `BluOr News Tracker <email>` — SEC blocks generic UAs.
...
### A.7 News engines
- Google News RSS: `https://news.google.com/rss/search?q=<q>&hl=en-US&gl=US&ceid=US:en`; run 3 date buckets by appending `when:1y` / `when:5y` to `q` (bare query is recency-biased). UA `BluOrNewsTracker/1.0`. Item `<link>` on `site:` queries is an opaque redirect — resolve via A.8.
